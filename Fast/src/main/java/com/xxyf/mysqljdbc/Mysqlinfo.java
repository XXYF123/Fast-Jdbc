...
public class Mysqlinfo {
    ...
    public Mysqlinfo(String driver, String url, String username, String password) {
        ...
        if (!Strif.strnull(url)) {
            if (!Strif.strnull(Mysqlinfo.url)) {
                Mysqlinfo.url = "jdbc:mysql://localhost:3306/noto?useSSL=false&useUnicode=true&characterEncoding=utf8";
            }
        } else {
            Mysqlinfo.url = url;
        }
    ...
    public void initpr(InputStream in) throws Jdbcinfo {
        ...
        if (Strif.strnull(properties.getProperty("buffsize"))) {
            buffsize = Integer.parseInt(properties.getProperty("buffsize"));
            if (buffsize <= 0) {
                buffsize = 50000;
            }
        }
